...
- **Tailwind CSS** — fast, consistent, no fighting a CSS file that's grown a mind of its own.
...
- **Prisma ORM** — schema-driven, type-safe database access, comfortable running PostgreSQL in production and SQLite for instant local dev.
- **NextAuth.js** — credentials-based auth with bcrypt hashing, OAuth providers, and role-based access control done right, not bolted on as an afterthought.
- **Stripe integration** — real payment flows, Checkout sessions, and the edge cases that come with handling money.
...
- **Jest & Vitest** — unit and integration tests that actually cover the tricky logic (overlap detection, race conditions, permission boundaries), not just the happy path.
- **Environment-based configuration** — projects that run the same way locally and in production, with sane switches between SQLite and Postgres.
- **Git & GitHub workflows** — clean history, meaningful commits, and a workflow built for collaboration.
...
![Ayoub's GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=default)
...
- Portfolio: _add link once live_
- Email: _add if you want it public_
